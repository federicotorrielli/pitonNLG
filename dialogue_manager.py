...
class DialogueManager:
    ...
    def wait_for_user_input(self) -> None:
        ...
        if "yesno" in self.current_state:
            self.resolve_yesno()
        else:
    ...
    def generate_hint_question(self) -> str:
        ...
        if phrase_ingredient:
            if phrase_validity:
                self.current_state = "yesno-correct"
                self.__ingredient_trick = current_ingredient
            ...
            if choice < self.trabocchetto[self.current_mental_state]:
                ...
                self.current_state = "yesno-trick"
                self.__ingredient_trick = current_ingredient
    ...
    def resolve_yesno(self) -> None:
        if self.current_state == "yesno-trick":
            ...
            if self.analized_phrase.yesno == "yes":
                # TODO: qua dovremmo dirgli che e' un coglione e non doveva dire di si (poi dobbiamo continuare a chiedere)
                pass
            else:
                # TODO: qua dovremmo congratularci con lui (non e' caduto nella trappola) (poi dobbiamo continuare a chiedere)
                pass
        elif self.current_state == "yesno-correct":
            self.current_state = "questions"
            if self.analized_phrase.yesno == "yes":
                # TODO: qua dovremmo congratularci con lui (ha fatto giusto) (poi dobbiamo continuare a chiedere)
                pass
            else:
                # TODO: qua dovremmo dirgli che e' un coglione e doveva dire di si (poi dobbiamo continuare a chiedere)
                pass
